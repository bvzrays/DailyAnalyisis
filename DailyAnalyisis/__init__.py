...
@on_core_start
async def initialize_daily_analyisis() -> None:
    await runtime.initialize()


@on_core_shutdown
async def shutdown_daily_analyisis() -> None:
    await runtime.shutdown()


@archive_sv.on_message(unique_id="daily_analyisis_archive", block=False)
...
@admin_sv.on_command(("概览", "overview"), block=True)
async def show_overview(bot: Bot, event: Event) -> None:
