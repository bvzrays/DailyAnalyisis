...
| LLM 与每日群漫画 | 插件专用 API Key、端点、模型、LLM 调参，以及漫画开关、目标群、角色、参考图、绘图后端、Provider 和重试 |
...
WebConsole 参数区按“总览、基础配置、LLM 配置、展示配置”拆分为 4 个原生配置标签；其中基础配置、LLM 配置和展示配置内部仍按功能分隔，并保留全部 schema 字段。插件 WebUI 同样按三栏导航展示，LLM 与每日群漫画配置集中在一起，不需要在单一长表单中查找。配置保存时，插件专用配置与 GsCore 配置会自动同步；从 GsCore 配置中心修改后建议重载插件。
...
- LLM 调用由插件内置的 OpenAI 兼容 HTTP Provider 执行，不借用 GsCore 全局 AI Provider 配置。
